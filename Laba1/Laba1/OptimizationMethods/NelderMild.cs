...
namespace Laba1.OptimizationMethods
{
    public class NelderMild
    {
        ...
        public Tuple<List<List<Point>>,double> FindMinimum(int iter, double eps)
		{
            List<List<Point>> allSimplex = new List<List<Point>>();
			...
			while (!IsItEnd(simplex, eps).Item1 && iterations < iter)
			{
                ...
                allSimplex.Add(new List<Point>() { new Point(simplex[0].x, simplex[0].y), new Point(simplex[1].x, simplex[1].y), new Point(simplex[2].x, simplex[2].y) });
            ...
            return new Tuple<List<List<Point>>, double>(allSimplex, Funct(simplex[1]));
		}
        ...
        private List<Point> ListSort(List<Point> points)
        {
            ...
            pointFunc = pointFunc.OrderBy(x => x.Item2).ToList();
